...
def generate_enterprise_admins_acls(session, domain_name):
    ...
    session.run(
        'MERGE (n:Group {name:$gname}) WITH n MERGE (m:Group {name:$ADgname}) WITH n,m MERGE (n)-[:MemberOf]->(m)',ADgname = get_name("ADMINISTRATORS",domain_name),  gname=group_name)
 
    # Ent Admins -> High Value Targets
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WriteDacl {isacl:true}]->(n)', nname=get_name("DOMAIN ADMINS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("DOMAIN CONTROLLERS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("SERVER OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WriteDacl {isacl:true}]->(n)', nname=get_name("ACCOUNT OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("PRINT OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("BACKUP OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("GROUP POLICY CREATOR OWNERS",domain_name), gname=group_name)
...
def generate_administrators_acls(session, domain_name):
    ...
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WriteDacl {isacl:true}]->(n)', nname=get_name("ENTERPRISE ADMINS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WriteDacl {isacl:true}]->(n)', nname=get_name("DOMAIN ADMINS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("DOMAIN CONTROLLERS",domain_name), gname=group_name)
...
def generate_domain_admins_acls(session, domain_name):
    ...
    # Added?
    session.run(
        'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:AllExtendedRights {isacl:true}]->(n)', domain=domain_name, gname=group_name)
 
    # Domain Admins -> High Value Targets
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:Owns {isacl:true}]->(n)', nname=get_name("ADMINISTRATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("PRINT OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("BACKUP OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:Owns {isacl:true}]->(n)', nname=get_name("ACCOUNT OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("SERVER OPERATORS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GenericWrite {isacl:true}]->(n)', nname=get_name("DOMAIN CONTROLLERS",domain_name), gname=group_name)
    session.run(
        'MERGE (n:Group {name:$nname}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:Owns {isacl:true}]->(n)', nname=get_name("ENTERPRISE ADMINS",domain_name), gname=group_name)
